...
final class RedirectBasedOnRole
{
     ...
     * Protects role-specific routes (/guru, /siswa).
    ...
    public function handle(Request $request, Closure $next): Response
    {
        if (Auth::check()) {
            ...
            if (($request->is('guru') || $request->is('guru/*')) && ! $user->hasRole('teacher')) {
                abort(403, 'Akses tidak diizinkan. Halaman ini hanya untuk guru.');
            }

            // Protect student routes - only students can access
            if (($request->is('siswa') || $request->is('siswa/*')) && ! $user->hasRole('student')) {
        ...
        if ($request->is('guru', 'guru/*', 'siswa', 'siswa/*', 'app', 'app/*')) {
