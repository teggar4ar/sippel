...
<div
    {{ $attributes->class(['border-t border-slate-100 bg-slate-50/40 px-3 py-2.5 dark:border-slate-700/80 dark:bg-slate-900/40 lg:px-5 lg:py-3']) }}>
    <div class="grid grid-cols-[1fr_auto] items-center gap-x-2 gap-y-2 lg:grid-cols-[1fr_auto_1fr]">
        <p class="order-2 text-[11px] leading-tight text-slate-500 dark:text-slate-400 lg:order-1 lg:text-xs">
            <span class="hidden sm:inline">Menampilkan </span><strong
                class="text-slate-700 dark:text-slate-200">{{ $paginator->firstItem() }}-{{ $paginator->lastItem() }}</strong>
            dari <strong class="text-slate-700 dark:text-slate-200">{{ $paginator->total() }}</strong>
            {{ $label }}
        </p>

        <div class="order-1 col-span-2 flex min-w-0 justify-center lg:order-2 lg:col-span-1">
            <div
                class="inline-flex max-w-full overflow-x-auto rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        @if ($perPage !== null)
            <div class="order-3 flex items-center justify-end gap-1.5 lg:gap-2">
                <label for="{{ $attributes->get('id', 'pagination-footer') }}-per-page"
                    class="text-[11px] leading-tight text-slate-500 dark:text-slate-400 lg:text-xs">Per halaman</label>
                <div class="relative">
                    <select id="{{ $attributes->get('id', 'pagination-footer') }}-per-page"
                    ...
                    <flux:icon variant="outline" name="chevron-down"
                        class="pointer-events-none absolute right-2 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 lg:right-2.5" />
                </div>
            </div>
        @else
