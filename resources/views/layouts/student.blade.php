...
<body class="min-h-screen bg-teal-50 dark:bg-slate-900 antialiased overflow-x-hidden"
    ...
    <div class="flex min-h-screen w-full overflow-x-hidden">
        ...
        <aside class="hidden lg:flex lg:flex-col lg:w-60 lg:fixed lg:inset-y-0 bg-teal-800 dark:bg-teal-950">
            {{-- Brand --}}
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                <a href="{{ route('student.dashboard') }}" wire:navigate
                   class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg transition-colors {{ request()->routeIs('student.dashboard') ? 'bg-teal-600 text-white' : 'text-teal-200 hover:bg-teal-700 hover:text-white' }}">
                    <flux:icon name="squares-2x2" variant="outline" class="w-5 h-5" />
                    Dashboard
                </a>
                <a href="{{ route('student.kehadiran') }}" wire:navigate
                   class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg transition-colors {{ request()->routeIs('student.kehadiran') ? 'bg-teal-600 text-white' : 'text-teal-200 hover:bg-teal-700 hover:text-white' }}">
                    <flux:icon name="calendar-days" variant="outline" class="w-5 h-5" />
                    Kehadiran
                </a>
                <a href="{{ route('student.nilai') }}" wire:navigate
                   class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg transition-colors {{ request()->routeIs('student.nilai') ? 'bg-teal-600 text-white' : 'text-teal-200 hover:bg-teal-700 hover:text-white' }}">
                    <flux:icon name="academic-cap" variant="outline" class="w-5 h-5" />
                    Nilai
                </a>
                <a href="{{ route('student.laporan') }}" wire:navigate
                   class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg transition-colors {{ request()->routeIs('student.laporan') ? 'bg-teal-600 text-white' : 'text-teal-200 hover:bg-teal-700 hover:text-white' }}">
                    <flux:icon name="chart-bar" variant="outline" class="w-5 h-5" />
                    Laporan
                </a>
                <a href="{{ route('student.profil') }}" wire:navigate
                   class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg transition-colors {{ request()->routeIs('student.profil') ? 'bg-teal-600 text-white' : 'text-teal-200 hover:bg-teal-700 hover:text-white' }}">
            ...
            {{-- User info --}}
            <div class="px-3 py-3 border-t border-teal-700">
                <div class="flex items-center gap-3 px-2 py-2">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-teal-300">Siswa</p>
                    </div>
                </div>
                <button onclick="event.preventDefault(); document.getElementById('logout-form').submit();"
                        class="flex items-center gap-2 w-full mt-2 px-3 py-2 text-sm text-teal-300 hover:text-white hover:bg-teal-700 rounded-lg transition-colors">
        ...
        <div x-data="{ open: false }" class="lg:hidden">
            ...
            <aside x-show="open" x-transition:enter="transition ease-out duration-200 transform"
                   ...
                   class="fixed inset-y-0 left-0 z-50 w-64 bg-teal-800 dark:bg-teal-950 shadow-xl">
                <div class="flex items-center justify-between h-14 px-4 bg-teal-900 dark:bg-black/30">
                    ...
                    <button @click="open = false" class="p-1.5 text-teal-300 hover:text-white rounded-lg">
                        <flux:icon name="x-mark" class="w-5 h-5" />
                    </button>
                </div>
                <nav class="px-3 py-4 space-y-1">
                    <a href="{{ route('student.dashboard') }}" wire:navigate @click="open = false"
                       class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg {{ request()->routeIs('student.dashboard') ? 'bg-teal-600 text-white' : 'text-teal-200 hover:bg-teal-700' }}">
                        <flux:icon name="squares-2x2" variant="outline" class="w-5 h-5" />
                        Dashboard
                    </a>
                    <a href="{{ route('student.kehadiran') }}" wire:navigate @click="open = false"
                       class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg {{ request()->routeIs('student.kehadiran') ? 'bg-teal-600 text-white' : 'text-teal-200 hover:bg-teal-700' }}">
                        <flux:icon name="calendar-days" variant="outline" class="w-5 h-5" />
                        Kehadiran
                    </a>
                    <a href="{{ route('student.nilai') }}" wire:navigate @click="open = false"
                       class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg {{ request()->routeIs('student.nilai') ? 'bg-teal-600 text-white' : 'text-teal-200 hover:bg-teal-700' }}">
                        <flux:icon name="academic-cap" variant="outline" class="w-5 h-5" />
                        Nilai
                    </a>
                    <a href="{{ route('student.laporan') }}" wire:navigate @click="open = false"
                       class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg {{ request()->routeIs('student.laporan') ? 'bg-teal-600 text-white' : 'text-teal-200 hover:bg-teal-700' }}">
                        <flux:icon name="chart-bar" variant="outline" class="w-5 h-5" />
                        Laporan
                    </a>
                    <a href="{{ route('student.profil') }}" wire:navigate @click="open = false"
                       class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg {{ request()->routeIs('student.profil') ? 'bg-teal-600 text-white' : 'text-teal-200 hover:bg-teal-700' }}">
                        <flux:icon name="user" variant="outline" class="w-5 h-5" />
                        Profil
                    </a>
                </nav>
                <div class="absolute bottom-0 left-0 right-0 px-3 py-3 border-t border-teal-700">
                    <div class="flex items-center gap-3 px-2 py-2">
                    ...
                    <button onclick="event.preventDefault(); document.getElementById('logout-form').submit();"
                            class="flex items-center gap-2 w-full mt-2 px-3 py-2 text-sm text-teal-300 hover:text-white hover:bg-teal-700 rounded-lg">
                        <flux:icon name="arrow-right-start-on-rectangle" variant="outline" class="w-4 h-4" />
                        Keluar
                    </button>
                </div>
            ...
            <header class="fixed top-0 left-0 right-0 z-30 flex items-center justify-between h-14 px-3 bg-teal-800 dark:bg-teal-900 lg:hidden safe-area-inset">
                <div class="flex items-center gap-2">
                    <button @click="open = true" class="p-2 -ml-1 text-teal-200 hover:text-white rounded-lg">
