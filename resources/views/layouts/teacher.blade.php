...
<body class="min-h-screen bg-slate-50 dark:bg-slate-950 antialiased overflow-x-hidden" data-inactivity-timer
    ...
    <div class="flex min-h-screen w-full overflow-x-hidden">
            ...
            <div class="px-3 py-3 border-t border-slate-700/80">
                <div class="flex items-center gap-3 px-2 py-2">
                    <div class="w-9 h-9 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0">
                        <span
                            class="text-sm font-semibold text-white">{{ substr(auth()->user()->name ?? 'G', 0, 1) }}</span>
                    </div>
        ...
        <div x-data="{ open: false }" class="lg:hidden">
            ...
            <aside x-show="open" x-transition:enter="transition ease-out duration-200 transform"
                ...
                <div class="absolute bottom-0 left-0 right-0 px-3 py-3 border-t border-slate-700/80">
                    <div class="flex items-center gap-3 px-2 py-2">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-white truncate">{{ auth()->user()->name ?? 'Guru' }}
                            </p>
                            <p class="text-xs text-slate-300">Guru</p>
                        </div>
                    </div>
            ...
            <header
                class="fixed top-0 left-0 right-0 z-30 flex items-center justify-between h-14 px-3 bg-slate-800 dark:bg-slate-900/95 lg:hidden safe-area-inset">
                <div class="flex items-center gap-2">
        ...
        <main class="flex-1 lg:pl-60 overflow-x-hidden">
            <div class="pt-16 lg:pt-4 pb-24 lg:pb-4 px-3 lg:px-6">
                <div class="max-w-6xl mx-auto w-full">
                    ...
                    @if (session('success'))
                        <div
                            class="mb-3 p-3 bg-emerald-50 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-200 rounded-lg text-sm">
                            {{ session('success') }}
                        </div>
                    @endif
                    ...
                    @if (session('error'))
                        <div
                            class="mb-3 p-3 bg-red-50 dark:bg-red-900/50 text-red-700 dark:text-red-200 rounded-lg text-sm">
                            {{ session('error') }}
                        </div>
                    @endif
        ...
        <x-nav.bottom-bar variant="teacher" :fab="['href' => route('teacher.aktivitas.create'), 'icon' => 'plus', 'label' => 'Buat Aktivitas']" :items="[
            [
            ...
            [
                ...
                'label' => 'Aktivitas',
                'active' => request()->routeIs('teacher.aktivitas.*'),
            ],
            [
                'href' => route('teacher.laporan'),
                'icon' => 'chart-bar',
                'label' => 'Laporan',
                'active' => request()->routeIs('teacher.laporan'),
            ],
            [
                'href' => route('teacher.profil'),
                'icon' => 'user-circle',
                'label' => 'Profil',
                'active' => request()->routeIs('teacher.profil'),
            ],
        ]" />
    </div>
    ...
    @include('partials.pwa-service-worker', ['swUrl' => '/sw-teacher.js', 'swLabel' => 'Teacher'])
</body>
