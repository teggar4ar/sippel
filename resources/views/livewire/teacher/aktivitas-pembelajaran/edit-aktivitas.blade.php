<div class="space-y-3 overflow-x-hidden">
    ...
    <div class="flex items-center gap-3">
        ...
        <div class="min-w-0">
            <h1 class="text-xl font-bold text-slate-900 dark:text-white">Edit Aktivitas</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Ubah data aktivitas pembelajaran</p>
        </div>
    </div>
    ...
    @if($aktivitas->absensi_mandiri)
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="px-3 py-2 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
                ...
                @if($aktivitas->activeSesi() && $aktivitas->activeSesi()->isActive())
                    @php
                    ...
                    <div x-data="qrCountdown({{ $expiresAt }}, {{ $remainingSeconds }})"
                         x-bind:class="{ 'bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300': remainingSeconds > 120, 'bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300': remainingSeconds <= 120 && remainingSeconds > 60, 'bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300 animate-pulse': remainingSeconds <= 60 }"
                         class="inline-flex items-center gap-1 text-xs px-2 py-1 rounded-lg font-semibold">
            ...
            <div class="p-3 space-y-2">
                @if($aktivitas->hasActiveSesi())
                    <div class="flex items-start gap-2 p-2 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-800">
                        <flux:icon name="check-circle" class="w-4 h-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0 mt-0.5" />
                        <div class="flex-1 min-w-0">
                            <p class="text-xs font-medium text-emerald-700 dark:text-emerald-300">Sesi Aktif</p>
                            <p class="text-[10px] text-emerald-600 dark:text-emerald-400 mt-0.5">
                                Dibuka {{ $aktivitas->activeSesi()->created_at->diffForHumans() }}
                                • Ditutup otomatis {{ $aktivitas->activeSesi()->expires_at->format('H:i') }}
                            </p>
                        </div>
                        <button wire:click="closeSession"
                                wire:loading.attr="disabled"
                                class="px-3 py-1.5 text-xs font-medium bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors flex-shrink-0 disabled:opacity-50 cursor-pointer">
                            <flux:icon wire:loading.remove wire:target="closeSession" name="x-mark" class="w-3.5 h-3.5 inline" />
                            <flux:icon wire:loading wire:target="closeSession" name="arrow-path" class="w-3.5 h-3.5 inline animate-spin" />
                            <span wire:loading.remove wire:target="closeSession">Tutup</span>
                            <span wire:loading wire:target="closeSession">Menutup...</span>
                        </button>
                    </div>
                @else
                    <div class="flex items-start gap-2 p-2 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-200 dark:border-slate-700">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button wire:click="openNewSession(5)"
                                wire:loading.attr="disabled"
                                class="flex-1 min-w-[100px] px-3 py-2 text-xs font-medium bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors disabled:opacity-50 cursor-pointer">
                            <flux:icon wire:loading.remove wire:target="openNewSession(5)" name="clock" class="w-3.5 h-3.5 inline" />
                            <flux:icon wire:loading wire:target="openNewSession(5)" name="arrow-path" class="w-3.5 h-3.5 inline animate-spin" />
                            5 Menit
                        </button>
                        <button wire:click="openNewSession(10)"
                                wire:loading.attr="disabled"
                                class="flex-1 min-w-[100px] px-3 py-2 text-xs font-medium bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors disabled:opacity-50 cursor-pointer">
                            <flux:icon wire:loading.remove wire:target="openNewSession(10)" name="clock" class="w-3.5 h-3.5 inline" />
                            <flux:icon wire:loading wire:target="openNewSession(10)" name="arrow-path" class="w-3.5 h-3.5 inline animate-spin" />
                            10 Menit
                        </button>
                        <button wire:click="openNewSession(15)"
                                wire:loading.attr="disabled"
                                class="flex-1 min-w-[100px] px-3 py-2 text-xs font-medium bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors disabled:opacity-50 cursor-pointer">
                            <flux:icon wire:loading.remove wire:target="openNewSession(15)" name="clock" class="w-3.5 h-3.5 inline" />
                            <flux:icon wire:loading wire:target="openNewSession(15)" name="arrow-path" class="w-3.5 h-3.5 inline animate-spin" />
                            15 Menit
                        </button>
                    </div>
                @endif
    ...
    <div class="space-y-3">
        ...
        <div class="max-h-[55vh] overflow-y-auto space-y-2 pr-1 -mr-1" x-data="{ expanded: null }">
            @foreach($this->siswaList as $siswa)
                ...
                @endphp
                <div
                    class="bg-white dark:bg-slate-800 rounded-lg border {{ $isUnset ? 'border-amber-300 dark:border-amber-700' : 'border-slate-200 dark:border-slate-700' }} overflow-hidden"
                ...
                >
                    ...
                    <div class="flex items-center gap-2 p-3">
                        {{-- Student info --}}
                        <div
                            class="flex items-center gap-2 flex-1 min-w-0 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 -m-2 p-2 rounded-lg transition-colors"
                            x-on:click="expanded = expanded === {{ $siswa->id }} ? null : {{ $siswa->id }}"
                        >
                            <div class="flex-1 min-w-0">
                                <div class="font-medium text-sm text-slate-900 dark:text-white truncate">{{ $siswa->user->name }}</div>
                                <div class="text-xs text-slate-500">{{ $siswa->nis }}</div>
                            </div>
                            ...
                            <flux:icon
                                name="chevron-down"
                                class="size-4 text-slate-400 transition-transform shrink-0"
                        ...
                        <div class="flex gap-1 shrink-0">
                            @foreach(['Hadir', 'Izin', 'Sakit', 'Alpa'] as $status)
                                ...
                                <button
                                    ...
                                    class="w-8 h-8 rounded-md text-xs font-bold transition-all {{ $isActive ? $btnColor : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600 cursor-pointer' }}"
                                    title="{{ $status }}"
                                >
                                    {{ substr($status, 0, 1) }}
                                </button>
                    ...
                    >
                        <div class="p-3 space-y-3">
                            ...
                            <div class="flex gap-2 {{ !$isHadir ? 'opacity-40 pointer-events-none' : '' }}">
                                <div class="flex-1">
                                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Nilai</label>
                                    <flux:input
                                        type="number"
                                        ...
                                        placeholder="0-100"
                                        :disabled="!$isHadir"
                                        class:input="h-9 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 focus:border-blue-500 focus:outline-none focus-visible:outline-none focus:ring-0 focus-visible:ring-0"
                                    />
                                </div>
                                <div class="flex-1">
                                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Partisipasi</label>
                                    <flux:select
                                        wire:model="detailAktivitas.{{ $siswa->id }}.partisipasi"
                                        :disabled="!$isHadir"
                                        class="h-9 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 focus:border-blue-500 focus:outline-none focus-visible:outline-none focus:ring-0 focus-visible:ring-0"
                                    >
                                        <option value="">-</option>
                                        @for($i = 1; $i <= 5; $i++)
                            ...
                            {{-- Notes --}}
                            <div>
                                <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Catatan</label>
                                <flux:input
                                    type="text"
