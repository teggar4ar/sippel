<div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{{ now()->translatedFormat('l, d F Y') }}</p>
        </div>
    ...
    <div class="grid grid-cols-3 gap-2 lg:gap-4">
        <div class="p-3 lg:p-4 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
            <div class="flex items-center gap-2 lg:gap-3">
                <div class="w-9 h-9 lg:w-10 lg:h-10 bg-blue-100 dark:bg-blue-900/50 rounded-lg flex items-center justify-center flex-shrink-0">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 truncate">Aktivitas</p>
                </div>
            </div>
        </div>
        <div class="p-3 lg:p-4 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
            <div class="flex items-center gap-2 lg:gap-3">
                <div class="w-9 h-9 lg:w-10 lg:h-10 bg-emerald-100 dark:bg-emerald-900/50 rounded-lg flex items-center justify-center flex-shrink-0">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 truncate">Kehadiran</p>
                </div>
            </div>
        </div>
        <div class="p-3 lg:p-4 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
            <div class="flex items-center gap-2 lg:gap-3">
                <div class="w-9 h-9 lg:w-10 lg:h-10 bg-amber-100 dark:bg-amber-900/50 rounded-lg flex items-center justify-center flex-shrink-0">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 truncate">Mapel</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="flex gap-2 lg:hidden">
        <a href="{{ route('teacher.aktivitas.list') }}" wire:navigate
           class="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-lg">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                ...
                <div class="flex items-center justify-between px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-xs text-slate-500 dark:text-slate-400">{{ $mapel->kelas->siswa_count ?? 0 }} siswa</p>
                        </div>
                    </div>
                    <a href="{{ route('teacher.aktivitas.create', ['mapel' => $mapel->id]) }}" wire:navigate
                       class="p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors flex-shrink-0">
        ...
        <div class="px-4 py-2 bg-slate-50 dark:bg-slate-700/50 text-center">
            ...
            @foreach($this->partisipasiPerKelas->take(4) as $data)
                <div>
                    <div class="flex justify-between text-sm mb-1.5">
                        <span class="text-slate-600 dark:text-slate-400 truncate mr-2">{{ $data['kelas'] }} · {{ $data['mapel'] }}</span>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 text-center">
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400 mb-4">Mulai catat aktivitas pembelajaran pertama Anda</p>
        <a href="{{ route('teacher.aktivitas.create') }}" wire:navigate
