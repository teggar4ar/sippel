<div class="space-y-4 overflow-x-hidden">
    {{-- Greeting --}}
    <x-ui.section-heading variant="student" title="Hai, {{ explode(' ', auth()->user()->name ?? 'Siswa')[0] }}!"
    ...
    @if ($this->siswa)
        ...
        <div
            class="p-3 bg-gradient-to-r from-teal-50 to-emerald-50 dark:from-teal-900/50 dark:to-emerald-900/50 rounded-xl border border-teal-200 dark:border-teal-700/90">
        ...
        <x-ui.card variant="student">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end gap-3 mb-4">
                <div class="flex items-center gap-2">
                    <div>
                        <label
                            class="block text-[11px] font-semibold uppercase tracking-wide text-teal-600 dark:text-teal-400 mb-1">Dari</label>
                    ...
                    <div>
                        <label
                            class="block text-[11px] font-semibold uppercase tracking-wide text-teal-600 dark:text-teal-400 mb-1">Sampai</label>
                        <input type="date" wire:model.live="tanggalSelesai"
                            class="w-full sm:w-auto px-3 py-2 text-sm bg-white dark:bg-slate-800 border border-teal-200 dark:border-slate-700 rounded-xl text-slate-700 dark:text-slate-200 focus:outline-none focus:border-teal-400 focus:ring-1 focus:ring-teal-400 transition" />
                    </div>
                </div>
                <div class="flex items-center gap-1.5">
                    <button wire:click="$set('filterCepat', 'semester')"
                    ...
                    <button wire:click="$set('filterCepat', 'bulan')"
                        class="px-3 py-1.5 rounded-full text-xs font-medium transition-colors cursor-pointer
                    ...
                    <button wire:click="$set('filterCepat', 'minggu')"
                        class="px-3 py-1.5 rounded-full text-xs font-medium transition-colors cursor-pointer
            ...
            <div class="lg:col-span-3 space-y-4">
                    ...
                    @if (!$heatmap['enrolled'])
                        <div class="p-6 text-center">
                            <flux:icon name="exclamation-triangle"
                                class="w-8 h-8 mx-auto text-amber-400 dark:text-amber-500 mb-2" />
                            <p class="text-sm text-teal-500 dark:text-teal-300">Kamu belum terdaftar di kelas untuk
                                tahun ajaran ini</p>
                        </div>
                    @else
                        {{-- Legend --}}
                        <div
                            class="flex flex-wrap gap-x-3 gap-y-1.5 px-4 py-3 text-xs text-teal-600 dark:text-teal-400">
                            ...
                            <span class="flex items-center gap-1 whitespace-nowrap">
                                <span
                                    class="size-3 rounded-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 shrink-0"></span>
                                Akan Datang
                            </span>
                        </div>
                        ...
                        <div class="overflow-x-auto scroll-smooth w-full pb-4">
                            ...
                            <div class="flex px-4 w-max min-w-full h-fit self-start">
                                ...
                                <div class="shrink-0 mr-2">
                                    ...
                                    <div class="grid grid-rows-5 gap-[3px]">
                                        @foreach (['Sen', 'Sel', 'Rab', 'Kam', 'Jum'] as $dayLabel)
                                            <div
                                                class="h-5 w-7 text-xs leading-4 text-teal-500 dark:text-teal-400 flex items-center justify-end">
                                                {{ $dayLabel }}</div>
                                        @endforeach
                                    </div>
                                </div>
                                ...
                                <div class="flex-1 min-w-0">
                                        ...
                                        @foreach ($heatmap['weeks'] as $week)
                                            <div class="w-5 shrink-0 relative h-full">
                                                @if ($week['is_first_week_of_month'])
                                                    <span
                                                        class="absolute bottom-0 left-0 z-10 whitespace-nowrap text-[10px] font-semibold text-teal-600 dark:text-teal-400">{{ $week['month_label'] }}</span>
                                                @endif
                                            </div>
                                        @endforeach
                                    </div>
                                    ...
                                    {{-- Flat cells grid --}}
                                    <div class="grid grid-flow-col grid-rows-5 gap-[3px]"
                                        style="grid-auto-columns: 20px">
                                        @foreach ($heatmap['weeks'] as $week)
                                            @foreach ($week['days'] as $day)
                                                <div class="size-5 rounded-sm {{ $cellColors[$day['status']] }}"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <x-ui.card variant="student" title="🔥 Streak Kehadiran" flush>
                        <x-slot:actions>
                            <span
                                class="text-[10px] font-medium uppercase tracking-wider text-teal-500 dark:text-teal-400">Berturut-turut</span>
                        </x-slot:actions>
                        <div class="p-4">
                            {{-- Streak Counter --}}
                            <div class="flex items-center gap-3 mb-4">
                                <div
                                    class="w-14 h-14 bg-gradient-to-br from-teal-400 to-emerald-500 rounded-xl flex items-center justify-center shadow-sm">
                                    <span
                                        class="text-2xl font-extrabold text-white leading-none">{{ $streak }}</span>
                                </div>
                                <div>
                            ...
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                {{ $streak >= $ms['target']
                                    ...
                                    : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500' }}">
                                        <span
                                            class="{{ $streak >= $ms['target'] ? '' : 'grayscale opacity-40' }}">{{ $ms['icon'] }}</span>
                                        <span>{{ $ms['label'] }}</span>
                                    </div>
                                @endforeach
                            </div>
                            ...
                            @if ($nextMilestone)
                                <div class="mt-3">
                                    <div class="flex items-center justify-between text-xs mb-1">
                                        <span class="text-teal-600 dark:text-teal-400">Menuju
                                            {{ $nextMilestone['icon'] }} {{ $nextMilestone['label'] }}</span>
                                        <span
                                            class="font-semibold text-teal-800 dark:text-white">{{ $streak }}/{{ $nextMilestone['target'] }}</span>
                                    </div>
                                    <div class="w-full h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                                        <div class="h-full bg-gradient-to-r from-teal-400 to-emerald-500 rounded-full transition-all duration-500"
                                            x-data="{ w: @js(min(round(($streak / $nextMilestone['target']) * 100), 100)) }" x-bind:style="`width: ${w}%`"></div>
                                    </div>
                                </div>
                            @else
                                <div class="mt-3 text-center">
                                    <p class="text-xs text-teal-500 dark:text-teal-400 font-medium">🎉 Semua milestone
                                        tercapai! Luar biasa!</p>
                                </div>
                            @endif
                        </div>
                    </x-ui.card>
                            ...
                            @foreach ($this->mataPelajaranList as $data)
                                ...
                                <div class="border border-teal-100 dark:border-slate-700/90 rounded-lg p-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <p class="text-sm font-semibold text-teal-900 dark:text-white truncate"
                                            title="{{ $data['nama_mapel'] }}">
                                            {{ $data['nama_mapel'] }}
                                        </p>
                                    </div>
                                    <div class="flex items-start justify-between gap-4 mt-3">
                                        <div class="flex-1 min-w-0">
                                            <div
                                                class="flex items-center gap-1.5 text-xs text-teal-700 dark:text-teal-200">
                                                <flux:icon name="users" class="w-3.5 h-3.5" />
                                                <span class="font-medium">Kehadiran</span>
                                                <span
                                                    class="font-semibold text-teal-900 dark:text-white">{{ $data['attendance_pct'] }}%</span>
                                            </div>
                                            <div
                                                class="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden mt-1.5">
                                                <div class="h-full bg-emerald-500 rounded-full"
                                                    x-data="{ w: @js(min($data['attendance_pct'], 100)) }" x-bind:style="`width: ${w}%`"></div>
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-1.5">
                                            <flux:icon name="star" class="w-3.5 h-3.5 text-amber-500" />
                                            <span class="text-xs text-teal-700 dark:text-teal-200">Keaktifan</span>
                                            <flux:badge size="sm" color="{{ $keaktifanColor }}">
                                                {{ $data['keaktifan_label'] }}</flux:badge>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    @else
                        <div class="p-6 text-center">
                            <flux:icon name="chart-bar"
                                class="w-8 h-8 mx-auto text-teal-300 dark:text-teal-500 mb-2" />
    ...
    @else
        ...
        <div class="p-4 bg-amber-50 dark:bg-amber-900/40 rounded-xl border border-amber-200 dark:border-amber-700/90">
            <div class="flex items-center gap-2">
                ...
                <p class="text-sm text-amber-800 dark:text-amber-200">Data siswa belum terhubung. Hubungi
                    administrator.</p>
            </div>
        </div>
    @endif
