<div class="space-y-3 overflow-x-hidden">
    ...
    <div>
        ...
        @if($siswa && $siswa->kelas)
            <p class="text-sm text-teal-600 dark:text-teal-400">Kelas {{ $siswa->kelas->tingkat_kelas }}-{{ $siswa->kelas->grup_kelas }}</p>
        @endif
    </div>
    ...
    @if($siswa)
        ...
        <div class="p-3 bg-gradient-to-r from-teal-50 to-emerald-50 dark:from-teal-900/30 dark:to-emerald-900/30 rounded-xl border border-teal-200 dark:border-teal-800">
            <div class="flex items-center gap-2">
                <flux:icon name="light-bulb" class="w-5 h-5 text-teal-500 flex-shrink-0" />
                <p class="text-sm text-teal-700 dark:text-teal-300">{{ $message['text'] }}</p>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-teal-900/30 rounded-xl shadow-sm border border-teal-200 dark:border-teal-800 overflow-hidden">
            <div class="flex">
                ...
                <div class="flex-1 py-3 text-center">
                    ...
                    <div class="text-[10px] text-teal-600 dark:text-teal-400">🔥 Streak</div>
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('student.nilai') }}" wire:navigate
               class="flex-1 py-2.5 text-center text-sm font-medium text-teal-700 dark:text-teal-200 bg-teal-100 dark:bg-teal-800 rounded-lg">
                Nilai
            </a>
        </div>
                ...
                @foreach($this->performancePerMapel as $data)
                    <div>
                        <div class="flex justify-between text-xs mb-1">
                            <span class="text-teal-700 dark:text-teal-300 truncate mr-2">{{ $data['nama_mapel'] }}</span>
                            <span class="font-bold text-teal-900 dark:text-white flex-shrink-0">{{ number_format($data['avg_nilai'], 1) }}</span>
                        </div>
                        <div class="w-full bg-teal-100 dark:bg-teal-800 rounded-full h-1.5">
            ...
            <div class="divide-y divide-teal-50 dark:divide-teal-800">
                @forelse($recentAktivitas as $detail)
                    <div class="p-3">
                        <div class="flex items-center justify-between gap-2 mb-1">
                            <span class="text-xs text-teal-500 dark:text-teal-400">{{ $detail->aktivitasPembelajaran->tanggal->format('d M') }}</span>
                            @php
                                $badgeColors = [
                        ...
                        <div class="flex items-center gap-2 mt-0.5 text-xs text-teal-600 dark:text-teal-400">
                ...
                @empty
                    <div class="p-6 text-center">
                        <flux:icon name="inbox" class="w-8 h-8 mx-auto text-teal-300 dark:text-teal-600 mb-2" />
                        <p class="text-sm text-teal-500 dark:text-teal-400">Belum ada aktivitas</p>
                    </div>
                @endforelse
            </div>
            @if($recentAktivitas->isNotEmpty())
                <a href="{{ route('student.kehadiran') }}" wire:navigate
                   class="block p-2 text-center text-xs text-teal-600 dark:text-teal-400 hover:bg-teal-50 dark:hover:bg-teal-800/50 border-t border-teal-100 dark:border-teal-800">
                    Lihat Semua →
                </a>
            @endif
        </div>
