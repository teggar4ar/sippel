<div class="flex items-center gap-2 min-w-0" x-data="{}"
    ...
    <div
        class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg {{ $variant === 'teal' ? 'bg-teal-50 dark:bg-teal-900/30' : 'bg-slate-100 dark:bg-slate-800' }}">
        <flux:icon name="calendar"
            class="w-4 h-4 {{ $variant === 'teal' ? 'text-teal-600 dark:text-teal-400' : 'text-slate-500 dark:text-slate-400' }}" />
    </div>
    <flux:select wire:model.live="selectedTahunAjaranId"
